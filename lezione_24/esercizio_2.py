...
class RecipeManager:
    ...
    def create_recipe(self, name: str, ingredients: list[str]) -> str | dict[str, list[str]]:
        if name in self.recipes:
            return "La ricetta già esiste"
        self.recipes[name] = ingredients
        return {name: self.recipes[name]}
    
    def add_ingredient(self, recipe_name: str, ingredient: str) -> str | dict[str, list[str]]:
        if recipe_name not in self.recipes:
            return "La ricetta non esiste"  # ERRORE: era "già esiste"
        elif ingredient in self.recipes[recipe_name]:
            return "L'ingrediente già esiste"
        else:
            self.recipes[recipe_name].append(ingredient)
            return {recipe_name: self.recipes[recipe_name]}
    
    def remove_ingredient(self, recipe_name: str, ingredient: str) -> str | dict[str, list[str]]:
        if recipe_name not in self.recipes:
    ...
    def update_ingredient(self, recipe_name: str, old_ingredient: str, new_ingredient: str) -> str | dict[str, list[str]]:
        if recipe_name not in self.recipes:
            return "La ricetta non esiste"
    ...
    def list_ingredients(self, recipe_name: str) -> str | list[str]:
        if recipe_name not in self.recipes:
            return "La ricetta non esiste"
        return self.recipes[recipe_name]
    
    def search_recipe_by_ingredient(self, ingredient: str) -> str | list[str]:
        ricette_trovate = []  
        for key, value in self.recipes.items():
            if ingredient in value:
                ricette_trovate.append(key)
